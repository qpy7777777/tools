...
import os
ont = FontProperties("SimHei")  # 定义一个字体对象
def time_plot(audio,fs):
    librosa.display.waveplot(audio,fs)
def spectrogram(audio,fs):
    data = librosa.stft(audio)
    librosa.display.specshow(librosa.power_to_db(abs(data)),x_axis="time",y_axis="mel",sr=fs)
...
if __name__=="__main__":
    ...
    for file in glob.glob(path+"/*.wav"):
        ...
        print(f'音频名称为{file},音频采样率为{fs}')
        audio_list.append(X)
        title_list.append(os.path.basename(file))
        fs_list.append(fs)
    cols,rows = 2,3
    figure = plt.figure(figsize=(12,8))
    for i in range(1,len(audio_list)+1):
        figure.add_subplot(cols,rows,i)
        time_plot(audio_list[i-1],fs_list[i-1])
    ...
    for i in range(1, len(audio_list)+1):
        figure.add_subplot(cols,rows, i)
        spectrogram(audio_list[i - 1], fs_list[i-1])
        plt.title(title_list[i - 1])
    plt.suptitle("时频图比较", fontproperties=font, fontsize=20)  # 添加整个图表的标题
    plt.tight_layout()
    plt.show()
    figure = plt.figure(figsize=(12, 8))
    for i in range(1, len(audio_list)+1):
        figure.add_subplot(cols,rows, i)
        magnitude_scalogram(audio_list[i - 1], fs_list[i-1])
        plt.title(title_list[i - 1])
    plt.suptitle("小波图比较", fontproperties=font, fontsize=20)  # 添加整个图表的标题
    plt.tight_layout()
    plt.show()
    for i in range(1, len(audio_list) + 1):
        wpd_plt(audio_list[i - 1], n=3)
        # 绘制小波包能量图
        # wp = pywt.WaveletPacket(data=audio, wavelet='db6',mode='symmetric')
        # print(f"wp.maxlevel{wp.maxlevel}")
        wp = pywt.WaveletPacket(data=audio_list[i - 1], wavelet='db1', mode='symmetric', maxlevel=3)  # 选用db1小波，分解层数为3
        n = 3
        re = []  # 第n层所有节点的分解系数
    ...
    path = "ShpsEar/original_shipsEar/0.wav"
    audio, fs = librosa.load(path, sr=None)
    cA, cD = pywt.dwt(audio, wavelet="db6", mode='smooth')
